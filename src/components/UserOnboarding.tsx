...
export default function UserOnboarding({ onComplete }: UserOnboardingProps) {
  ...
  useEffect(() => {
    const loadUserData = async () => {
      ...
      try {
        const existingUser = await getUserByEmail(currentUser.email) as any;
        if (existingUser) {
          // Pre-populate form with existing data
          setGender(existingUser.gender || '');
          setAge(existingUser.age ? existingUser.age.toString() : '');
          setHeight(existingUser.height || 0);
          setWeight(existingUser.weight || 0);
          setBust(existingUser.bust || 0);
          setWaist(existingUser.waist || 0);
          setHip(existingUser.hip || 0);
  ...
  if (isLoadingData) {
    return (
      <div className="w-full max-w-2xl mx-auto bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 border border-gray-200 dark:border-gray-700">
        <div className="text-center">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500 mx-auto"></div>
          <p className="text-gray-600 dark:text-gray-300 mt-2">Loading your profile...</p>
        </div>
      </div>
    );
  }
